...
3. 确认 `miniprogram/config.js` 中的 `resourceAppid`、`envId` 和 `service` 与你的云托管一致。
4. 如果小程序和云托管环境不属于同一 AppID，需要先在云托管控制台完成资源复用授权。
5. 当前示例通过 `wx.cloud.callContainer` 调用云托管，不需要配置 `request 合法域名`。
